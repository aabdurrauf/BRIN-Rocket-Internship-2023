...
def layer(input_layer, num_next_neurons, is_output=False, activation=tf.nn.relu):
    ...
    biases = tf.get_variable("biases", [num_next_neurons], initializer=tf.keras.initializers.glorot_normal)
...
class Utils():
    ...
    def normalize(self, state):
        # print("this: ", self.normalizer.transform(np.array(state).reshape(1, -1)).flatten())
