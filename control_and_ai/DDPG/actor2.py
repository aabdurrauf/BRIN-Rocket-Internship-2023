...
class Actor():
    ...
    def get_action(self, state, explore=True):
        actions = self.sess.run(self.infer,
                                feed_dict={self.state_ph: state, self.phase: True})
        ...
        if explore:
            for action in actions:
                for i in range(len(action)):
                    ...
                    # bound it to the action space, action[3] is for throttle so the lower bound is 0 not -1
                    action[i] = max(min(action[i], self.action_space_bounds[i]), 0)
        return actions
