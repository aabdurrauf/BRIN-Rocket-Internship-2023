...
class RocketLander(gym.Env):
    ...
    def __init__(self, settings):
        ...
        self.action_space = [0, 0, 0, 0]       # Throttle, Elevator, Aileron, Rudder
    ...
    def _set_rocket(self, client):
        # set position (altitude = 1000 meters)
        X = -998
        self.values = [X, X, 1000, X, X, X, X]
        client.sendPOSI(self.values, 0)
    ...
    def _step(self, action, client):
        assert len(action) == 4  # Throttle, Elevator, Aileron, Rudder

        # Check for contact with the ground ==>>> we can set if altitude =< 0 set the FLAG to True
        ...
        # TODO: tomorrow check these part, main engines stuff, compute reward, reset

        # receive the control data and append it to the action_history
        ...
        state, self.untransformed_state = self.__generate_state(client)  # Generate state
        self.state = state  # Keep a record of the new state

        # Rewards for reinforcement learning
        reward = self.__compute_rewards(state, throttle, elevator,
                                        aileron)  # part angle can be used as part of the reward
    ...
    def __generate_state(self, client):
        # ----------------------------------------------------------------------------
        # Update
        # self.world.Step(1.0 / FPS, 6 * 30, 6 * 30)
        alti = client.getDREF('sim/cockpit2/gauges/indicators/radio_altimeter_height_ft_pilot')[0]
        posx = client.getDREF('sim/flightmodel/position/local_x')[0]
        # pos y is the altitude of the rocket
        posz = client.getDREF('sim/flightmodel/position/local_z')[0]
        ver_vel = client.getDREF('sim/flightmodel/position/vh_ind')[0]
        ...
        state = [alti, posx, posz, ver_vel, pitch, roll]  # we have to normalize this state first using target
        untransformed_state = [alti, posx, posz, ver_vel, pitch, roll]
